...
SamplerState PointClamp : register(s1); // ¾Æ¿ô¶óÀÎ¿ë Point/Clamp »ùÇÃ·¯
...
{
    float4 color = g_CartoonTex.Sample(PointClamp, input.worldPos.xy);
    float4 outline = float4(0, 0, 0, 0);
    for (int i = 0; i < 8; i++)
    {
        float2 offset = float2(cos(i * 0.25f * 3.14159265f), sin(i * 0.25f * 3.14159265f));
        float4 sample = g_CartoonTex.Sample(PointClamp, input.worldPos.xy + OutlineThickness * offset);

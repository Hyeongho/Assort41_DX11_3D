...
StructuredBuffer<matrix> g_OffsetArray : register(t16);
StructuredBuffer<FrameTrans> g_ChangeFrameTransArray : register(t17);
RWStructuredBuffer<matrix> g_BoneMatrixArray : register(u0);
RWStructuredBuffer<OutputBoneInfo> g_BoneSocketMatrixArray : register(u1);
...
void AnimationUpdateCS(int3 ThreadID : SV_DispatchThreadID)
{
    ...
    g_BoneSocketMatrixArray[ThreadID.x].matBone = transpose(matBone);

    g_BoneSocketMatrixArray[ThreadID.x].Scale = Scale.xyz;
    g_BoneSocketMatrixArray[ThreadID.x].Rot = Rot;
    g_BoneSocketMatrixArray[ThreadID.x].Pos = Pos.xyz;
